...
function NewDrinkForm(props){
  ...
  function handleNewDrinkFormSubmission(event) {
    ...
    props.onNewDrinkCreation({name: event.target.name.value, brand: event.target.brand.value, flavor: event.target.flavor.value, price: event.target.price.value, pints : 124, id: v4()});
  }
